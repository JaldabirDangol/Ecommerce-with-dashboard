...
const ReviewSection = async ({ id }: { id: string }) => {
  ...
  return (
    <div className="w-full flex flex-col space-y-6">
      ...
      <div className="w-full flex flex-col p-6 bg-white rounded-lg shadow-md border border-gray-200">
        <div className="mb-4 text-center">
          <h2 className="text-2xl font-bold text-gray-800">
            Ratings & Customer Reviews
          </h2>
          <p className="flex items-center justify-center text-gray-700 text-sm mt-1">
      ...
      {userId ? (
        hasUserReviewed ? (
          <div className="w-full text-center p-6 bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            >
              <div className="flex items-center gap-1 mb-2 text-yellow-400">
                {Array.from({ length: review.rating || 0 }).map((_, i) => (
                  <span key={`filled-${review.id}-${i}`}><FaStar /></span>
                ))}
                {Array.from({ length: 5 - (review.rating || 0) }).map((_, i) => (
                  <span key={`empty-${review.id}-${i}`} className="text-gray-300"><FaStar /></span>
                ))}
              </div>
